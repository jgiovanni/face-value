			...
			<form @submit.prevent="updateProfile">
				<div class="row">
					<div class="col col-lg-6 col-md-6 col-sm-12 col-12">
						<b-form-group class="form-group label-floating" :class="{ 'is-empty': !userData.name }"
						              :state="errorState('name')"
						              :invalid-feedback="errors.first('name')"
						              label="Your Full Name" label-class="control-label" label-for="name">
							<b-input v-validate="'required'" v-model="userData.name"
							         :state="errorState('name')"
							         id="name" name="name" type="text"></b-input>
						</b-form-group>

						<b-form-group class="form-group label-floating" :class="{ 'is-empty': !userData.program }"
						              :state="errorState('program')"
						              :invalid-feedback="errors.first('program')"
						              label="Your Program/Major" label-class="control-label" label-for="program">
							<b-form-select id="program" name="program" v-model="userData.program" class="mb-3" style="height: auto;" >
								<optgroup label="Undergraduate Programs">
									<option v-for="opt in programOptions.undergraduate" :key="opt" :value="opt">{{ opt }}</option>
								</optgroup>
								<optgroup label="Graduate Programs">
									<option v-for="opt in programOptions.graduate" :key="opt" :value="opt">{{ opt }}</option>
								</optgroup>
							</b-form-select>
						</b-form-group>
					...
					<div class="col col-lg-6 col-md-6 col-sm-12 col-12">
						...
						<b-form-group class="form-group label-floating" :class="{ 'is-empty': !userData.email }"
						              :state="errorState('email')"
						              :invalid-feedback="errors.first('email')"
						              label="Your Email" label-class="control-label" label-for="email">
							<b-input v-validate="'required'" v-model="userData.email"
							         :state="errorState('email')" disabled
					...
					<div class="col col-12">
						<b-form-group class="form-group label-floating" :class="{ 'is-empty': !userData.about }"
						              :state="errorState('about')"
						              :invalid-feedback="errors.first('about')"
						              label="Write a little description about you" label-class="control-label" label-for="about">
							<b-textarea v-validate="'min:3'" v-model="userData.about"
							            :state="errorState('about')"
							            id="about" name="about"></b-textarea>
						</b-form-group>
					...
					<div class="col col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12">
						<b-form-group class="form-group with-icon label-floating" :class="{ 'is-empty': !userData.website }"
						              ...
						              label="Your Website/Portfolio" label-class="control-label" label-for="name">
							<b-input v-validate="'min:3'" v-model="userData.website"
							         :state="errorState('website')" data-vv-as="Website/Portfolio"
							         id="website" name="website" type="url"></b-input>
							<i class="fa fa-globe" style="top: 11px;" aria-hidden="true"></i>
						</b-form-group>

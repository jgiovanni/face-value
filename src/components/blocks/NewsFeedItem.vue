<template>
	<div class="ui-block">
		<article class="hentry post">
			...
			<div class="post__author author vcard inline-items">
				...
				<div class="author-date">
					<router-link class="h6 post__author-name fn" :to="`/${item.author.userName}`"
					             v-text="item.author.displayName"></router-link>
			...
			<div class="post-additional-info inline-items">
				...
				<a href="#" @click.prevent="likePost" class="post-add-icon inline-items" :aria-disabled="userIsAuthor"
				   :class="{ 'disabled': userIsAuthor, 'active': liked }">
				...
				<div class="names-people-likes">
					<Aggregator :item="item" count-prop="like_count" :user-included="liked" recents-prop="recent_likes"
					            verb="liked" />
				</div>

				<div class="comments-shared">
					<a href="#" @click.prevent="startConversation" class="post-add-icon inline-items"
					   :class="{'active': hasChatted}" v-if="!userIsAuthor">
			...
			<div class="control-block-button post-control-button">
				...
				<a href="#" @click.prevent="likePost" class="btn btn-control" :aria-disabled="userIsAuthor"
				   :class="{ 'disabled': userIsAuthor, 'active': liked }">
				...
				<a href="#" @click.prevent="startConversation" class="btn btn-control" :class="{'active': hasChatted}"
				   v-if="!userIsAuthor">
		...
		<!-- Comment Form  -->
		<form v-show="showMessageForm" class="comment-form inline-items" @submit.prevent="sendMessage"
			...
			<div class="post__author author vcard inline-items">
				...
				<div class="form-group with-icon-right ">
					...
					<div class="add-options-message">
						<a href="#" class="options-message" @click.prevent="" data-toggle="modal"
    ...
    methods: {
      likePost() {
        if (!this.userIsAuthor) {
          if (this.liked) {
            // un-like
            this.$db.collection("likes").doc(`${this.user.id}_${this.item.id}`).delete().catch(console.log);

            this.$db.collection("newsFeed").doc(this.item.id).collection("likes").doc(this.liked.newsFeedLike).delete().catch(console.log);
            this.item.like_count--;
          } else {
            // like
            this.$db.collection("newsFeed").doc(this.item.id).collection("likes").add({
              ...
              this.$db.collection("likes").doc(`${this.user.id}_${this.item.id}`).set({
                user: this.user.id,
